...
4. installs fisher and syncs the fish plugins (`install/fish-plugins.sh`),
5. sets `fish` as the login shell.
...
`fish/` carries the parts of `~/.config/fish/` that are actually yours:

| Tracked | |
|---------|---|
| `config.fish` | PATH, aliases, starship, zoxide, **vi mode** |
| `functions/` | `note`, `worklog`, `xclip-kb`, `azlogin`, `bazel-resync`, `compile-commands`, `fish_user_key_bindings` |
| `conf.d/fish_frozen_theme.fish` | the colour scheme |
| `fish_plugins` | the fisher plugin list |

**Vi mode lives in `config.fish` on purpose.** fish 4.3 migrated
`fish_key_bindings` out of universal variables into a generated
`conf.d/fish_frozen_key_bindings.fish`, which is machine-local — so a new
machine came up with no vi mode and no `[N]`/`[I]` indicator. `config.fish` is
the only copy that travels, which is also what fish's own migration note
recommends. If that generated file is still lying around, it's redundant; delete
it.
...
completions that fzf.fish, nvm.fish and bass generate are not tracked. Bootstrap
reproduces them:

```sh
./install/fish-plugins.sh    # installs fisher, then `fisher update`
```

It has to run *after* the stow step, since fisher reads `fish_plugins` out of
`$HOME`. `fisher update` also removes anything not on the list, so add plugins
by editing the file rather than by running `fisher install`.

### Not tracked, on purpose

| File | Why |
|------|-----|
| `functions/fish_prompt.fish`, `fish_right_prompt.fish` | written by `conda init`; they call `__fish_prompt_orig`, which doesn't exist without conda, so copying them to a new machine breaks the prompt outright |
| `functions/bat.fish` | wraps `batcat`, which doesn't exist on a box where bat came from cargo. `install_shims` in `tools.sh` handles the rename properly |
| `functions/kitty.fish` | hardcodes `~/ext/kitty/...` and shadows the real kitty that `config.fish` already puts on PATH |
| `conf.d/rustup.fish` | generated by rustup |
| `fish_variables` | fish rewrites it constantly; it's machine state, not config |
